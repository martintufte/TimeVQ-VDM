...
class SynthethicDataset(Dataset):
...
class VDM(pl.LightningModule):
    def __init__(
        ...
        self.n_samples_seen = torch.zeros(self.model.n_classes)# number of samples seen for each class
    ...
    def pred_start_from_v(self, z, t, v):
        """
    ...
    def model_predictions(self, z, t, condition=None):
        ...
        if self.objective == 'pred_noise':
            ...
            pred_v = pred
            pred_x = self.pred_start_from_v(z, t, pred_v)
            pred_noise = self.pred_noise_from_start(z, t, pred_x)
    ...
    @torch.no_grad()
    def sample_dataset(self, n_samples=10, sampling_steps=100, guidance_weight=1.0, class_probs=None):
        """ 
        Ancestral sampling from the diffusion model.
        the class labels are drawn with the same probability as the model is trained on.
        """
        class_probs = default(class_probs, self.n_samples_seen / torch.sum(self.n_samples_seen))
        class_distribution = Categorical(class_probs)
        
        Y = class_distribution.sample((n_samples, 1)).to(self.device)
        X = self.sample(n_samples, sampling_steps, Y.view(-1), guidance_weight)
        
        return SynthethicDataset(X, Y)
    ...
    def p_losses(self, x, t=None, condition=None, noise=None):
        ...
        if self.objective == 'pred_noise':
            ...
            # calculate velocity
            sigma_t = self.sigma(t).view(-1,1,1).to(self.device)
            alpha_t = self.alpha(t).view(-1,1,1).to(self.device)
            target = - sigma_t * x + alpha_t * noise
        else:
