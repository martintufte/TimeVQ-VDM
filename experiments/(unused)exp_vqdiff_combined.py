# -*- coding: utf-8 -*-
"""
Created on Sun Apr 23 14:55:29 2023
...
class ExpVQDiffCombined(ExpBase):
    def __init__(self,
                 config: dict,
                 n_train_samples: int,
                 stage: int = 2):
    ...
    def forward(self, batch):
        ...
        diffusion_loss = self.diffusion(z_q, y)
        
        return recons_loss, vq_losses, perplexities, diffusion_loss



    def training_step(self, batch, batch_idx):
        recons_loss, vq_losses, perplexities, diffusion_loss = self.forward(batch)
    ...
    def configure_optimizers(self):
        opt = torch.optim.AdamW([{'params': self.encoder.parameters(), 'lr': self.config['stage1']['lr']},
                                 ...
                                 {'params': self.diffusion.parameters(), 'lr': self.config['stage1']['lr']}
                                 ],
