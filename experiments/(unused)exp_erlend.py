# -*- coding: utf-8 -*-
"""
Created on Mon Apr 24 14:18:51 2023
...
class ExpErlend(ExpBase):
    def __init__(self,
        ...
        self.scaler = StandardScaler().fit(torch.cat((embed_l, embed_h), dim=1))
        ...
        self.unet = Unet(
            ...
            dim          = config['Unet']['dim'],
            dim_mults    = config['Unet']['dim_mults']['lf'],
            in_channels  = config['VQ']['codebook_dim'] * 10,
    ...
    def sample(self, n_samples: int, sampling_steps_lf: int, sampling_steps_hf: int, class_index=None, batch_size=256, guidance_scale=1.):
        ...
        for i in tqdm(range(0, n_samples, batch_size), desc='Sampling:', total = n_iters):
            ...
            # sample LF part
            z = self.diffusion.sample(b, sampling_steps_lf, class_index, guidance_scale) # (B C (H W))
    ...
    def forward(self, batch):
        ...
        if self.stage == 1:
            ...
            # stack low- and high-frequency model
...
if __name__ == "__main__":
    ...
    chans_high = 4
    batch = 128
    low_freq = torch.randn(batch, chans_low, n_low)
    high_freq = torch.randn(batch, chans_high, n_high)

    noise_low = torch.randn_like(low_freq)  # Scale with diffusion parameter
    noise_high = torch.randn_like(high_freq)  # Scale with diffusion parameter

    in_low = low_freq + noise_low
    upsampler = nn.ConvTranspose1d(chans_low, out_channels=chans_high, kernel_size=4, stride=4)
    unet_input_low = upsampler.forward(in_low)
    unet_input_high = high_freq + noise_high
    unet_input = torch.cat((unet_input_high, unet_input_low), dim=1)
    print('Unet input shape:', unet_input.shape)
    unet = nn.Identity()  # replace with actual unet

    unet_out = unet.forward(unet_input)

    downsampler = nn.Conv1d(2*chans_high, chans_low, kernel_size=4, stride=4)

    low_out = downsampler.forward(unet_out)

    projector_high = nn.Conv1d(2*chans_high, chans_high, kernel_size=1, stride=1)

    high_out = projector_high.forward(unet_out)

    print('Unet output low:', low_out.shape)
    print('Unet output hihg:', high_out.shape)
