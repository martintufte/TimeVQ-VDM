# -*- coding: utf-8 -*-
"""
Created on Sun Apr 30 11:26:02 2023
...
from utils import exists, compute_downsample_rate, freeze, timefreq_to_time, time_to_timefreq, \
    zero_pad_low_freq, zero_pad_high_freq, quantize, get_root_dir


class ExpVQVAEDiffusionSmooth(ExpBase):
    def __init__(self,
                 config: dict,
                 n_train_samples: int,
                 stage: int = 2):
        ...
        self.scaler_l = None #StandardScaler(mean=embed_l.mean(), std=embed_l.std())
        self.scaler_h = None #StandardScaler(mean=embed_h.mean(), std=embed_h.std())
        ...
        self.unet_l = Unet(
            ...
            in_channels  = config['VQ']['codebook_dim'] * floor(1 + config['VQ']['n_fft']/2),
        ...
        # High frequency smoothing diffusion model
        self.unet_h = Unet(
            ts_length    = diff_length_h,
            n_classes    = self.n_classes,
            dim          = config['Unet']['dim'],
            dim_mults    = config['Unet']['dim_mults']['hf'],
            in_channels  = config['VQ']['codebook_dim'] * floor(1 + config['VQ']['n_fft']/2),
            resnet_block_groups = config['Unet']['resnet_block_groups'],
            time_dim     = config['Unet']['time_dim'],
            class_dim    = config['Unet']['class_dim'],
        ...
        if self.stage > 2:
            for module_name, module in self.modules['stage2'].items():
                ...
                module.eval()
        if self.stage > 3:
            for module_name, module in self.modules['stage3'].items():
                self.load(module, get_root_dir().joinpath('saved_models'), f'{module_name}-{self.dataset_name}.ckpt')
                freeze(module)
                module.eval()
    ...
    def sample(self, n_samples: int, sampling_steps_lf: int, sampling_steps_hf: int, class_index=None, batch_size=256, guidance_scale=1.):
        ...
        for i in range(0, n_samples, batch_size):
            ...
            # sample LF part
            z_l = self.diffusion_l.sample(b, sampling_steps_lf, class_index, guidance_scale, 'LF') # (B C (H W))
            ...
            # sample HF part
            z_h = self.diffusion_h.sample(b, sampling_steps_hf, class_index, guidance_scale, 'HF') # (B C (H W))
