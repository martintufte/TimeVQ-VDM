...
def train_stage(config: dict,
    ...
    # fit
    train_exp = ExpVQVAEDiffusion(config, len(train_data_loader.dataset), stage)
...
if __name__ == '__main__':
    ...
    # train
    if stage < 3:
        train_stage(config, stage, train_data_loader)
    else:
        model = ExpVQVAEDiffusion(config, len(train_data_loader.dataset), stage)


    # sampler
    n_samples = 100
    sampling_steps_lf = 50
    sampling_steps_hf = 10
    class_index = 0
    batch_size = 128
    guidance_scale = 2.0
